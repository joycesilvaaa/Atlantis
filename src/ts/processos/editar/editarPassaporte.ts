...
export default class EditarPassaporte extends Processo{
    ...
    processar(): void {
        console.log('Inciando Atualização de Passaporte...')
        ...
        const documentosLista = ListaDocumentosPorTipo(documentosAtuais, TipoDocumento.Passaporte);
        if (!documentosLista) {
            console.log('Nenhum documento Passaporte cadastrado.');
            return;
        }
        ...
        while (true){
            ...
            let continuar = this.entrada.receberTexto('Deseja atualizar mais algum Passaporte? (S/N)').toLowerCase().trim()
            if(continuar === 'n'){
                break
            }
        }
