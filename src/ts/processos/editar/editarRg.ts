...
export default class EditarRg extends Processo{
    ...
    processar(): void {
        console.log('Inciando Atualização de Rg...')
        ...
        const documentosLista = ListaDocumentosPorTipo(documentosAtuais, TipoDocumento.RG);
        if (!documentosLista) {
            console.log('Nenhum documento Rg cadastrado.');
            return;
        }
        ...
        while (true){
            ...
            let continuar = this.entrada.receberTexto('Deseja atualizar mais algum Rg? (S/N)').toLowerCase().trim()
            if(continuar === 'n'){
                break
            }
        }
